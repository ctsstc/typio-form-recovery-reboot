...
ui.inject = function (dataObj, replaceObj, callback) {
  ...
  if (dataObj.path) {
    ...
    if (replaceObj)
      dataObj.html = replacePlaceholders(dataObj.html, replaceObj);
...
function loadIconFont() {
  ...
  let html =
    '<style>@import "' +
    chrome.runtime.getURL("fonts/typio/styles.css") +
    '";</style>';
  innerShadowRootNode.insertAdjacentHTML("afterbegin", html);

  var iconfont = new FontFace(
    "typio",
    "url(" + chrome.runtime.getURL("fonts/typio/fonts/typio.woff") + ")",
  );
