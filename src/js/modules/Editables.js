...
function getEditable(el, onlyTextEditable = false) {
  ...
  if (el && editableCache.has(el)) {
    ...
    return !onlyTextEditable || (onlyTextEditable && ed.isTextEditable())
      ? ed
      : false;
  } else if (
    (!onlyTextEditable && editables.isEditable(el)) ||
    (onlyTextEditable && editables.isTextEditable(el))
  ) {
...
editables.isTextEditable = (elem) => {
  ...
  if (
    editables.isNode(elem, "INPUT") &&
    editables.isTextEditableType(elem.type)
  ) {
    return true;
  } else if (elem.contentEditable === "true") {
    return true;
