  ...
  node.addEventListener("mousedown", function (e) {
    ...
    if (e.target.dataset.hide !== undefined) {
      ...
      requestAnimationFrame(function () {
        quickAccessController.show(terafm.focusedEditable, {
          x: e.layerX,
          y: e.layerY,
        });
      });
...
function disableForEditable() {
  if (window.terafm.focusedEditable)
    disabledEditables.push(window.terafm.focusedEditable);
}
...
function show(trigger) {
  ...
  build(function () {
    ...
    if (
      triggerAction === "focus" &&
      !(parseInt(edStyle.width) > 80 && parseInt(edStyle.height) > 10)
    )
      return;
...
function positionIcon(editable) {
  ...
  if (
    editable.el.scrollHeight > editable.el.clientHeight ||
    ["search", "number"].includes(editable.type)
  ) {
    pos.x -= 17;
  }
