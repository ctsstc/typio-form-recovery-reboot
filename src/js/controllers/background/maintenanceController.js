...
function runMaintenance() {
  chrome.storage.sync.get("storageTimeDays", function (data) {
    let maxdays = data.hasOwnProperty("storageTimeDays")
      ? data["storageTimeDays"]
      : defaultOptions.get("storageTimeDays");
    ...
    chrome.storage.local.get(null, function (data) {
      ...
      for (let domain in data) {
        if (
          domain.indexOf("###") !== 0 ||
          !data[domain].hasOwnProperty("fields")
        )
          continue;
        let action = "ignore";
        ...
        if (action === "save") {
          console.log(
            "maintenance run on",
            domain /*, JSON.parse(JSON.stringify(data[domain]))*/,
          );
