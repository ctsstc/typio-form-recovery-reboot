<template>
  <div>
    ...
    <form @change="$store.dispatch('options/save')">
      <p>
        You may configure the keyboard shortcuts to your liking, but Typio
        cannot detect collisions with other existing keyboard shortcuts. If you
        encounter issues after changing the keyboard shortcuts, please change or
        disable them.
      </p>

      <fancy-check v-model="options.keybindEnabled"
        >Enable Enable keyboard shortcuts</fancy-check
      >
      <br />

      <div class="keyboard-shortcut-list">
        <keyboard-shortcut-item
          v-for="option of keyboardShortcutOptions"
          :key="option.key"
          :option="option"
          :value="options[option.key]"
          @click="openPicker(option)"
        ></keyboard-shortcut-item>
      </div>

      <p>
        The Quick Restore Popup can be navigated with the up/down arrow keys.
        Press space to select, or
        <span style="white-space: nowrap">Shift + Delete</span> to delete the
        selected item.
      </p>
    </form>
