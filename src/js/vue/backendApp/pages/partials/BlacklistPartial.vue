<template>
  <div>
    ...
    <p>
      Typio will be completely disabled on these domains. Autosave and
      restoration will not be available.
    </p>
    <p>Use * as a wildcard to block all subdomains, e.g: *.bank.com</p>
    <p>
      You can use Regex by wrapping the pattern in forward slashes /like this/.
      It will match against the hostname of the site.
    </p>

    <form @submit="onAdd" class="add-form">
      <input
        ref="domainToAdd"
        placeholder="Enter a domain name or a Regex to block"
        type="text"
      />
    ...
    <div ref="blockList" class="block-list">
      <div
        v-if="!blockedDomains || blockedDomains.length === 0"
        class="no-blocks-message"
      >
