  ...
  <div>
    ...
    <div class="card">
      <div style="background: rgb(226 226 226); padding: 15px">
        ...
        <p>
          The features on this page have NOT gone through extensive testing and
          might not function as expected. Use at your own risk.
        </p>
      </div>
      <br />
      ...
      <p>
        Please note that any existing data for the new domain will be replaced
        with the old - the data will *not* be merged.
      </p>

      <form @submit="changeDomainName">
        <label for="domainNameChangeInput">Change from:</label>
        <input
          id="domainNameChangeInput"
          v-model="domainChangeFrom"
          list="domainDatalist"
          placeholder="Current domain name"
          style="width: 200px"
        />
...
export default {
  ...
  methods: {
    ...
    importFromFile() {
      ...
      if (
        !confirm(
          "Importing a database will OVERRIDE the existing database. This is non-reversible. Do you want to continue?",
        )
      ) {
        return false;
      }
      ...
      fileReader.onload = function (e) {
        ...
        if (arr.length) {
          ...
          for (const domain of arr) {
            if (
              domain.hasOwnProperty("domainId") &&
              domain.hasOwnProperty("context")
            ) {
    ...
    exportToFile() {
      chrome.permissions.request(
        ...
        function (granted) {
          ...
          if (granted) {
            const blob = new Blob([JSON.stringify(buckets)], {
    ...
    changeDomainName(e) {
      ...
      if (this.domainExists(this.domainChangeFrom) !== true) {
        return alert(
          "There is no data saved for the selected domain. Did you spell it correctly?",
        );
      }
    ...
    applyEntryFilter() {
      if (this.entryFilter) {
        ...
        this.entryList = [
          buck.getEntries(null, null, (entry) => {
            return entry.valueContains(this.entryFilter) !== -1;
          }),
        ];
      }
