<template>
  <div
    ...
    v-on:click="backgroundClickHide($event)"
  >
    <div class="modal">
      <div class="header">
        <div class="top-bar">
        ...
        <div class="primary">
          <div class="left">Recover {{ hostname }}</div>
          <button class="toolbar-icon" v-on:click="openDonationLink()">
            <span class="icon-heart"></span>Support development
          </button>
          <button class="toolbar-icon" v-on:click="wipeData()">
            <span class="icon-trash"></span>Delete all data
          </button>
          <button class="toolbar-icon" v-on:click="disableSite()">
            <span class="icon-block"></span>Disable on this site
          </button>
          <button class="toolbar-icon" v-on:click="openOptions()">
            <span class="icon-gear"></span>Open options
          </button>
        </div>
      </div>
      ...
      <div class="panes">
        <div class="left">
          <div class="header">
            <div class="filter-box">
              <input
                class="filter-input typioIgnoreField"
                type="text"
                placeholder="Filter entries"
                v-model="filterText"
                v-on:input="populate(true)"
              />
              <div class="chk-label">
                <div class="pretty-chk">
                  <input
                    type="checkbox"
                    id="chk-hide-small-entries"
                    class="typioIgnoreField"
                    v-model="filterShowTextOnly"
                    v-on:change="updateOptsfilterShowTextOnly()"
                  />
                  <div class="fake-chk"></div>
                </div>
                <label for="chk-hide-small-entries"
                  >Hide non-text entries</label
                >
              </div>
              <span class="icon icon-search"></span>
            </div>
            <p class="filter-warning" v-if="filteredCount">
              {{ filteredCount }} entries hidden -
              <a
                v-on:click="
                  resetFilters();
                  populate(true);
              ...
              <template v-for="sess in sesslist.getArray().reverse()">
                <p v-if="sess.length" class="date-stamp">
                  ...
                  <template v-if="sess.getFirstEntry().originURL">
                    &nbsp;&middot;&nbsp;
                    <a
                      :href="sess.getFirstEntry().originURL"
                      :title="sess.getFirstEntry().originURL"
                      >view page</a
                    >
                  </template>
                </p>
                <ul v-if="sess.length" class="card-1">
                  <li
                    v-for="entry in sess.entries"
                    :data-session-id="entry.sessionId"
                    :data-editable-id="entry.editableId"
                    v-on:click="setEntry($event)"
                  >
                    <p v-html="entry.getPrintableValue({ truncate: 300 })"></p>
                    <div class="meta">
                      <div class="left">
                        <span v-if="entry.canBeAutoRestored()" class="status ok"
                          >Can be automatically restored</span
                        >
                        <span
                          v-if="!entry.canBeAutoRestored()"
                          class="status bad"
                          >Cannot be automatically restored
                          <a target="_blank" :href="noAutoRestoreHelpLink"
                            >(info)</a
                          ></span
                        >
                      </div>
                      <div class="right">
                        <a
                          class="delete"
                          v-on:click="deleteEntry($event)"
                          :class="delConfirmEntry === entry ? 'confirm' : ''"
                          >{{
                            delConfirmEntry === entry
                              ? "Click to confirm"
                              : "Delete"
                          }}</a
                        >
        ...
        <div class="right">
          <div
            ...
            v-bind:class="[page === 'default' || !page ? 'page-current' : '']"
          >
            <div class="center">
              ...
              <p>
                Typio has saved <b>{{ stats.countTotEntries }} entries</b> in
                <b>{{ stats.countTotSessions }} sessions</b> with<br />a total
                size of <b>{{ stats.countTotSize }} megabytes</b> for this
                domain.
              </p>
              <p>
                Psst. Did you know about the
                <a v-on:click="openKeyboardShortcuts()">keyboard shortcuts</a>?
              </p>
            </div>
          </div>

          <div
            class="page page-entry"
            v-bind:class="[page === 'entry' ? 'page-current' : '']"
            v-if="currEntry"
          >
            <div class="entry-header">
              <template v-if="currEntry.canBeAutoRestored()">
                <button class="btn btn-primary" v-on:click="restoreSession()">
                  Restore session
                </button>
                <button class="btn btn-flat" v-on:click="restoreEntry()">
                  Restore only this
                </button>
              </template>
              ...
              <template v-if="currEntry.type === 'contenteditable'">
                <div
                  style="float: right"
                  class="btn-drop-container"
                  @click="
                    getEventTarget($event)
                      .closest('.btn-drop-container')
                ...
                >
                  <button
                    class="btn"
                    v-bind:class="[
                      !currEntry.hasEditable() ? 'btn-primary' : 'btn-flat',
                    ]"
                  >
                    Copy &#9662;
                  </button>
              ...
              <template v-else>
                <button
                  style="float: right"
                  class="btn"
                  v-on:click="copyEntry('plaintext')"
                  v-bind:class="[
                    !currEntry.hasEditable() ? 'btn-primary' : 'btn-flat',
                  ]"
                >
              ...
              <p class="message-warn" v-if="!currEntry.canBeAutoRestored()">
                <span class="icon-info"></span> This entry cannot be restored
                automatically.
                <a target="_blank" :href="noAutoRestoreHelpLink">Why?</a>
              </p>
            </div>

            <iframe
              class="entry-text card-1"
              :srcdoc="iframeSrc"
              sandbox=""
            ></iframe>
            <div id="entry-path" class="entry-meta card-1">
              {{ currEntry.path }} &nbsp; (type {{ currEntry.type }}, input
              {{ currEntry.hasEditable() ? "found" : "not found" }})
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      filterText: "",
      noAutoRestoreHelpLink: chrome.runtime.getURL(
        "/html/app.html#/faq#cannot-auto-restore",
      ),
  ...
  computed: {
    iframeSrc() {
      const style =
        "<style>body { margin: 0; padding: 20px; font-family: sans-serif; font-size: 15px; color: #333; } body > *:first-child { margin-top: 0; }  img { max-width: 100%; }</style>";
  ...
  methods: {
    ...
    setEntry: function (e) {
      ...
      this.currEntry = this.sesslist.getEntry(
        target.dataset.sessionId,
        target.dataset.editableId,
      );
      this.page = "entry";
    ...
    restoreSession: function () {
      ...
      if (sess.length !== autoRestorableCount) {
        toastController.create(
          "Some entries could not be automatically restored (restored " +
            autoRestorableCount +
            " of " +
            sess.length +
            ")",
    ...
    populate: function (opts = { scrollTop: false }) {
      ...
      db.fetch()
        .then(() => {
          ...
          db.getDomainSize().then(
            (bytes) =>
              (this.stats.countTotSize = Number(bytes / 1024 / 1024).toFixed(
                2,
              )),
          );
    ...
    deleteEntry: function (e) {
      ...
      entry = this.sesslist.getEntry(
        li.dataset.sessionId,
        li.dataset.editableId,
      );
    ...
    copyEntry: function (format) {
      ...
      if (format === "plaintext") {
        ...
        Helpers.copyToClipboard(
          this.currEntry.getValue({ trim: true, trimNewLines: true }),
        );
    ...
    wipeData: function () {
      if (
        ...
        toastController.create(
          "Database cleared for " + window.location.hostname,
        );
        this.hide();
      }
    },
