<template>
  <div id="quickAccess" v-bind:class="[!isVisible ? 'hidden' : '']">
    <p v-if="isEmpty" style="margin: 10px; font-weight: bold">
      No entries found.
    </p>

    <ul
      class="entry-list"
      v-for="(dataType, index) in Object.keys(data)"
      :key="index"
    >
      <entry-item
        v-for="(entry, i) in data[dataType].entries"
        :key="i"
        item-type="entry"
        :entry="entry"
        :editable="editable"
        :isSess="dataType === 'sess'"
      ></entry-item>
    </ul>
    ...
    <ul class="footer">
      <entry-item
        item-type="link"
        action="openRecovery"
        item-text="Browse all entries"
      ></entry-item>
      <entry-item
        item-size="short"
        item-tooltip="Show keyboard shortcuts"
        item-type="link"
        action="openKeyboardModal"
        item-img="icon-keyboard"
      ></entry-item>
      <entry-item
        item-size="short"
        item-tooltip="Disable Typio on this domain"
        item-type="link"
        action="disableTypio"
        item-img="icon-block"
      ></entry-item>
  ...
  methods: {
    showAndPopulate: function (ed, coord) {
      ...
      if (Options.get("qaGroupSessions")) {
        data.sess = db
          .getSessionsContainingEditable(
            window.terafm.focusedEditable.id,
            maxItems,
          )
          .getEntriesByEditable(window.terafm.focusedEditable.id, maxItems);
      }
      const numberOfRecentsToFetch = maxItems - data.sess.length;
      data.recent = db.getEntries(
        numberOfRecentsToFetch,
        window.terafm.focusedEditable.id,
        function (entry) {
