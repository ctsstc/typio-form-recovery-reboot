<template>
  <li
    v-if="itemType !== 'entry' || (itemType === 'entry' && entry.isTextType())"
  >
    <div
      @click="commit()"
      @mouseenter="select"
      @mouseleave="unselect"
      v-bind:class="[selected ? 'selected' : '', 'selectable', 'fill']"
      :data-tooltip="itemTooltip"
    >
      <template v-if="itemType === 'entry'">
        <span v-if="!isSess" class="icon inner-fake-arrow icon-arrow-forward"
          ><span
            data-tooltip="Restore this entry (this entry was typed in another field)"
          ></span
        ></span>
    ...
    <div
      data-tooltip="Restore just this entry."
      v-if="isSess"
      @click="commit(true)"
      @mouseenter="singleSelect"
      @mouseleave="unselect"
      v-bind:class="[
  ...
  methods: {
    ...
    commit(commitSingleFromSession) {
      if (this.itemType === "link") {
        ...
        else if (this.action === "openKeyboardModal")
      ...
      if (this.entry) {
        if (!this.isSess || commitSingleFromSession) {
          ...
          const editables = Object.values(
            this.entry.getSession().getEditables().editables,
          );
        }
      }
