...
export default class SessionList {
  ...
  push(session) {
    if (!(session instanceof Session))
      throw new Error("SessionList only accepts Sessions.");
    this.sessions[session.id] = session;
  }

  pushTo(sid, entry) {
    if (!(entry instanceof Entry))
      throw new Error("pushTo only accepts Entries.");
  ...
  getEntry(sid, eid) {
    ...
    return this.sessions[sid].entries.hasOwnProperty(eid)
      ? this.sessions[sid].entries[eid]
      : null;
  }

  merge(list) {
    if (!(list instanceof SessionList))
      throw new Error(
        "Merge requires another " + this.constructor.name + " to merge.",
      );
