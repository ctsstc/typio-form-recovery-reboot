...
export default class EntryList {
  ...
  set(data, opts = { override: false }) {
    ...
    } else if (data instanceof EditableList) {
      for (let eid in data.editables) {
        this.set(data.editables[eid], opts);
      }
    } else if (data instanceof Editable) {
      if (this.uniqueEditables) {
  ...
  containsEditable(checkEntry) {
    if (!this.uniqueEditables)
      throw new Error("Cannot check uniqueness for non-unique EntryList");
    return this.indexes.hasOwnProperty(checkEntry.editableId)
      ? this.indexes[checkEntry.editableId]
      : false;
  }
}
