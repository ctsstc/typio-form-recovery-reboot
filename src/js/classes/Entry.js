...
export default class Entry {
  ...
  valueContains(string) {
    return this.getValue({ stripTags: this.type === "contenteditable" })
      .replace(/\s/g, "")
      .toLowerCase()
      .indexOf(string.replace(/\s/g, "").toLowerCase());
  }
  ...
  resolveRadio(ed) {
    if (ed.type === "radio" && ed.el.checked === false) {
      let sel = ed.el
        .getRootNode()
        .querySelector('input[type=radio][name="' + ed.el.name + '"]:checked');
  ...
  getPrintableValue(opts) {
    ...
    if (entry.type === "radio" && entry.meta.meta) {
      ...
      value +=
        Helpers.encodeHTMLEntities(entry.meta.meta) +
        (entry.value == "1" ? " (checked)" : " (unchecked)");
    } else {
      if (entry.meta.meta) {
