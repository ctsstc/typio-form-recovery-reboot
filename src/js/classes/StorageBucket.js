...
export default class StorageBucket {
  ...
  get sessionIds() {
    return this._sessionIds !== undefined
      ? this._sessionIds
      : (this._sessionIds = this.generateSessionIds());
  }
  ...
  del(sid, eid) {
    if (
      this.fields.hasOwnProperty(eid) &&
      this.fields[eid].sess.hasOwnProperty(sid)
    ) {
  ...
  setEntry(entry) {
    ...
    this.fields[entry.editableId].sess[entry.sessionId] = {
      value: entry.value,
    ...
    allsess.each((sess) => {
      sess.each((entry) => {
        ...
        if (
          ...
          filterFn.call(null, entry) === false
        ) {
          return null;
        }
