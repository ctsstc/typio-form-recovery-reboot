...
(function() {
	let input = document.querySelector('#set-shortcut'),
		shortcut = ['Alt',  'd'],
		current = []
	...
	document.body.addEventListener('keydown', function(e) {
		...
		if(current.indexOf(e.key) === -1) {
			current.push(e.key);
		}

		if(shortcut.length === current.length) {
			var ok = true
			for(var i=0; i < shortcut.length; ++i) {
				if(current.indexOf(shortcut[i]) === -1) {
					ok = false;
					break;
				}
			}
			...
			if(ok) {
				console.log('MATCHES YO!', current, 'shortcut', shortcut, 'lengths', shortcut.length,  current.length)
			}
		}
